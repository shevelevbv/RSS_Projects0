...
function addAnimate(event) {
  if (event.target.classList.contains("theme__icon")) {
    document.querySelectorAll(".theme__icon").forEach((btn) => {
      ...
      document.querySelector(".theme").addEventListener("animationend", () => {
        document
          .querySelectorAll(".theme__icon")
          .forEach((icon) => icon.classList.remove("animate"));
      });
    });
  }
  if (event.target.classList.contains("portfolio__button")) {
    ...
    document
      .querySelector(".portfolio__images")
...
function changeImage(event) {
  if (event.target.classList.contains("portfolio__button")) {
    images.forEach(
      (img, index) =>
        (img.src = `./img/jpg/${event.target.dataset.season}/${index + 1}.jpg`)
    );
  }
}
...
document.querySelector(".theme").addEventListener("click", (event) => {
  changeTheme(event), addAnimate(event);
});
